...
export const TextPreprocessForm = component$(({ store, userName }: TextPreprocessFormProps) => {
  const submitTextAction = usePublishText();
  ...
  useTask$(({ track }) => {
    track(() => submitTextAction.value);

    if (submitTextAction.value?._id) {
      setTimeout(() => {
        nav("/read/unapproved-texts/" + submitTextAction.value?._id);
      }, 3000);
    }
  });
